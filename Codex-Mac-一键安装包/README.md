...
- 使用 Homebrew `node@24` 作为明确 LTS 目标；普通路径场景把 Codex 安装到 `brew --prefix node@24`，ASCII-safe 场景安装到专用根目录的 npm prefix。
- 将 Codex npm bin 和 `node@24/bin` 以受管区块写入 zsh/bash profile 的 PATH 前面，并在安装时验证该 PATH 可解析目标 `codex`。
- 清理旧安装器写入的 npm prefix/cache 配置，以及旧 ASCII-safe `CODEX_HOME` profile 导出。
...
- HOME/TMPDIR 含非 ASCII 时使用 ASCII-safe 根目录。自定义 `CODEX_UNIX_ASCII_ROOT` 仅允许 `/Users/Shared/Codex-<name>`；拒绝符号链接、非目录和非当前用户所有的已有目录。
